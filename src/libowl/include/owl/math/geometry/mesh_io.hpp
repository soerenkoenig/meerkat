...
namespace owl
{
  namespace math
  {
    namespace geometry
    {
      ...
      bool read_off(mesh<Scalar> &mesh, const std::string &p)
      {
...
//      utils::progress loading_progress(n);
        ...
        reader.listen_2_face([&mesh](const std::vector<std::size_t> &indices)
                             {
                               std::vector<vertex_handle> vertex_indices;
                               for (auto v: indices)
                                 vertex_indices.push_back(vertex_handle(v));
      ...
      template<typename Scalar>
      bool read_ply(mesh<Scalar> &mesh, const std::string &p)
      {
        if (!utils::file_exists(p))
          return false;
...
//          loading_progress.step(face_step_size);
        ...
        ply.listen_2_element_property<std::int32_t>("vertex", "material_index",
                                                    [&material_idx](const std::int32_t &id)
                                                    { material_idx = id; });
        ...
        ply.listen_2_element_property<float>("material", "refract_index",
                                             [](const float &refract_index)
                                             { std::cout << refract_index << " "; });
        ply.listen_2_element_property<float>("material", "extinct_coeff",
                                             [](const float &extinct_coeff)
                                             { std::cout << extinct_coeff << " "; });
      ...
      template<typename Scalar>
      bool read(mesh<Scalar> &mesh, const std::string &p)
      {
