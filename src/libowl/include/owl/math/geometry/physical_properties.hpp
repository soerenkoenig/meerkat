    ...
    namespace geometry
    {
      ...
      public:
        physical_properties(Scalar density = 1)
          : surface_area_{0}, density_{density}, moments{vector<Scalar, 10>::zero()}
        {
        ...
        void add_quad(const vector<Scalar, 3> &p1, const vector<Scalar, 3> &p2, const vector<Scalar, 3> &p3,
          const vector<Scalar, 3> &p4)
        {
        ...
        void add_triangle(const vector<Scalar, 3> &p1, const vector<Scalar, 3> &p2, const vector<Scalar, 3> &p3)
        {
          ...
          auto xy = ((p1.x() + p2.x() + p3.x()) * (p1.y() + p2.y() + p3.y()) + p1.x() * p1.y() + p2.x() * p2.y() +
                     p3.x() * p3.y()) / 24;

          auto xz = ((p1.x() + p2.x() + p3.x()) * (p1.z() + p2.z() + p3.z()) + p1.x() * p1.z() + p2.x() * p2.z() +
                     p3.x() * p3.z()) / 24;

          auto yz = ((p1.y() + p2.y() + p3.y()) * (p1.z() + p2.z() + p3.z()) + p1.y() * p1.z() + p2.y() * p2.z() +
                     p3.y() * p3.z()) / 24;
          ...
          auto x_3 =
            ((p1.x() + p2.x() + p3.x()) * (sqr(p1.x()) + sqr(p2.x()) + sqr(p3.x())) + p1.x() * p2.x() * p3.x()) / 20;
          auto y_3 =
            ((p1.y() + p2.y() + p3.y()) * (sqr(p1.y()) + sqr(p2.y()) + sqr(p3.y())) + p1.y() * p2.y() * p3.y()) / 20;
          auto z_3 =
            ((p1.z() + p2.z() + p3.z()) * (sqr(p1.z()) + sqr(p2.z()) + sqr(p3.z())) + p1.z() * p2.z() * p3.z()) / 20;

          auto x_2y = ((3 * p1.y() + p2.y() + p3.y()) * sqr(p1.x()) + (p1.y() + 3 * p2.y() + p3.y()) * sqr(p2.x()) +
                       (p1.y() + p2.y() + 3 * p3.y()) * sqr(p3.x()) +
                       (2 * p1.y() + 2 * p2.y() + p3.y()) * p1.x() * p2.x() +
                       (2 * p1.y() + p2.y() + 2 * p3.y()) * p1.x() * p3.x() +
                       (p1.y() + 2 * p2.y() + 2 * p3.y()) * p2.x() * p3.x()) / 60;
          auto x_2z = ((3 * p1.z() + p2.z() + p3.z()) * sqr(p1.x()) + (p1.z() + 3 * p2.z() + p3.z()) * sqr(p2.x()) +
                       (p1.z() + p2.z() + 3 * p3.z()) * sqr(p3.x()) +
                       (2 * p1.z() + 2 * p2.z() + p3.z()) * p1.x() * p2.x() +
                       (2 * p1.z() + p2.z() + 2 * p3.z()) * p1.x() * p3.x() +
                       (p1.z() + 2 * p2.z() + 2 * p3.z()) * p2.x() * p3.x()) / 60;

          auto y_2x = ((3 * p1.x() + p2.x() + p3.x()) * sqr(p1.y()) + (p1.x() + 3 * p2.x() + p3.x()) * sqr(p2.y()) +
                       (p1.x() + p2.x() + 3 * p3.x()) * sqr(p3.y()) +
                       (2 * p1.x() + 2 * p2.x() + p3.x()) * p1.y() * p2.y() +
                       (2 * p1.x() + p2.x() + 2 * p3.x()) * p1.y() * p3.y() +
                       (p1.x() + 2 * p2.x() + 2 * p3.x()) * p2.y() * p3.y()) / 60;
          auto y_2z = ((3 * p1.z() + p2.z() + p3.z()) * sqr(p1.y()) + (p1.z() + 3 * p2.z() + p3.z()) * sqr(p2.y()) +
                       (p1.z() + p2.z() + 3 * p3.z()) * sqr(p3.y()) +
                       (2 * p1.z() + 2 * p2.z() + p3.z()) * p1.y() * p2.y() +
                       (2 * p1.z() + p2.z() + 2 * p3.z()) * p1.y() * p3.y() +
                       (p1.z() + 2 * p2.z() + 2 * p3.z()) * p2.y() * p3.y()) / 60;

          auto z_2y = ((3 * p1.y() + p2.y() + p3.y()) * sqr(p1.z()) + (p1.y() + 3 * p2.y() + p3.y()) * sqr(p2.z()) +
                       (p1.y() + p2.y() + 3 * p3.y()) * sqr(p3.z()) +
                       (2 * p1.y() + 2 * p2.y() + p3.y()) * p1.z() * p2.z() +
                       (2 * p1.y() + p2.y() + 2 * p3.y()) * p1.z() * p3.z() +
                       (p1.y() + 2 * p2.y() + 2 * p3.y()) * p2.z() * p3.z()) / 60;
          auto z_2x = ((3 * p1.x() + p2.x() + p3.x()) * sqr(p1.z()) + (p1.x() + 3 * p2.x() + p3.x()) * sqr(p2.z()) +
                       (p1.x() + p2.x() + 3 * p3.x()) * sqr(p3.z()) +
                       (2 * p1.x() + 2 * p2.x() + p3.x()) * p1.z() * p2.z() +
                       (2 * p1.x() + p2.x() + 2 * p3.x()) * p1.z() * p3.z() +
                       (p1.x() + 2 * p2.x() + 2 * p3.x()) * p2.z() * p3.z()) / 60;

          auto xyz = ((p1.x() + p2.x() + p3.x()) * (p1.y() + p2.y() + p3.y()) * (p1.z() + p2.z() + p3.z()) -
                      (p2.y() * p3.z() + p3.y() * p2.z() - 4 * p1.y() * p1.z()) * p1.x() / 2 -
                      (p1.y() * p3.z() + p3.y() * p1.z() - 4 * p2.y() * p2.z()) * p2.x() / 2 -
                      (p1.y() * p2.z() + p2.y() * p1.z() - 4 * p3.y() * p3.z()) * p3.x() / 2) / 60;
