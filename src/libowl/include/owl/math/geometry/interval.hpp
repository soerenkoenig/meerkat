...
namespace owl
{
  namespace math
  {
    namespace geometry
    {
      ...
      {
        return out << (LowerBoundOpen ? "( " : "[ ") << inter.lower_bound << ", " << inter.upper_bound
                   << (UpperBoundOpen ? ")" : "]");
      }

      template<typename Scalar, std::size_t Dimension, bool LowerBoundOpen, bool UpperBoundOpen>
      std::ostream &
      operator<<(std::ostream &out, const interval<Scalar, Dimension, LowerBoundOpen, UpperBoundOpen> &inter)
      {
        return out << (LowerBoundOpen ? "( " : "[ ") << "(" << inter.lower_bound << ")" << ", (" << inter.upper_bound
                   << ") " << (UpperBoundOpen ? ")" : "]");
      }
