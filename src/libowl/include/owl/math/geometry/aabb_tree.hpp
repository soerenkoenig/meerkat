  ...
  namespace math
  {
    namespace geometry
    {
      ...
      public:
        ...
        aabb_tree(Deref
        deref = {}, std::size_t
        max_depth = 20, std::size_t
        min_size = 2, scalar
        box_offset_eps = 0.001)
        ...
        //copy constructor
        aabb_tree(
          const aabb_tree &other)
          : max_depth_(other.max_depth_), min_size_(other.min_size_),
            root_(other.root_ ? other.root_->clone() : nullptr), deref_(other.deref_)
        {
      ...
      protected:
        ...
        aabb compute_bounds(const std::vector<Primitive> &primitive_list) const
        {
          ...
          for (const auto &p : primitive_list)
            b.insert(geometry::bounds<scalar, dimension, false, false>(deref_(p)));
