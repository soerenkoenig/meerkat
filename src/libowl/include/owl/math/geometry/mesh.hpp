...
//        utils::progress progress(num_faces());
        ...
        {
          return utils::all_of(faces(), [this, &n](face_handle f)
          { return is_n_gon(f, n); });
        }
        ...
        box bounds() const
        {
          return geometry::bounds(positions(vertices()));
        }
 ...
 * , typename = std::enable_if_t<(... && std::is_same<std::decay_t<VertexHandles>,vertex_handle>::value)>*/>
      ...
      */
      ...
      template<typename Scalar>
      std::vector<line_segment<Scalar, 3>>
      compute_face_normal_line_segments(const mesh<Scalar> &m, const Scalar &length = Scalar{1})
      {
