...
#include "owl/math/ray_segment.hpp"
...
namespace owl
{
  namespace math
  {
    ...
    template <typename Primitive, typename Scalar = typename scalar_t<Primitive>::type,
      std::size_t Dimension = dimension_t<Primitive>::value>
    std::optional<vector<Scalar,Dimension>> closest_intersection_point(const Primitive& prim, const ray<Scalar,Dimension>& r)
    {
    ...
    template <typename Primitive, typename Scalar = typename scalar_t<Primitive>::type,
      std::size_t Dimension = dimension_t<Primitive>::value>
    std::optional<vector<Scalar,Dimension>> closest_intersection_point(const Primitive& prim, const ray_segment<Scalar,Dimension>& rs)
    {
