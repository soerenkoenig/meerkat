...
namespace owl
{
  ...
  {
  public:
    using vector = typename vector_t<Primitive>::type;
    using scalar = typename scalar_t<Primitive>::type;
    ...
    aabb_tree(std::size_t max_depth = 20, std::size_t min_size = 2,scalar box_offset_eps = 0.001)
      : max_depth_(max_depth)
      , min_size_(min_size)
      , root_(nullptr)
      , box_offset_eps_(box_offset_eps)
    {
    }
  ...
  protected:
    ...
    aabb_node* build(std::vector<Primitive>&& primitive_list, const aabb& b, std::size_t depth)
    {
      ...
      std::nth_element(primitive_list.begin(), mid, primitive_list.end(),
                       ...
                       { return reference_point(a)[axis] < reference_point(b)[axis];}
      );
